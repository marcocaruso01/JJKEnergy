...
function install(){if(root.__JJK_V392_INSTALLED__)return;root.__JJK_V392_INSTALLED__=true;const start=()=>{bind();setTimeout(bind,100);setTimeout(()=>{bind();getfn('renderAll')?.();decorate();audit();},500);setTimeout(bind,1500);const runtimeRefresh=()=>{patchTechniqueFunction();applyBlueprints();updatePanels();decorate();};root.addEventListener('jjk:runtime-scheduled',event=>{const kind=event.detail?.kind||'state';if(['state','remote','screen','all','rebind'].includes(kind)&&!document.hidden)setTimeout(runtimeRefresh,0);},{passive:true});root.addEventListener('pageshow',()=>setTimeout(runtimeRefresh,0),{passive:true});};document.readyState==='loading'?document.addEventListener('DOMContentLoaded',start,{once:true}):start();}
